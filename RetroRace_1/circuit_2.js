...
class tileMap {
     ...
     * @param {Number} height le nombre de tile en hauteure
     * @param {Number} width le nombre de tile en largeure 
     * @param {Number} tileSide la taille des tiles en pixel
     * @param {Number[]} tilesImg un tableau qui contient l id de l image
     */
    constructor(mapX, mapY, height, width, tileSide, tilesImg, scale = 0) {
        this.mapX = mapX;
        this.mapY = mapY;
        this.height = height;
        this.width = width;
        ...
        /*
                if (scale) {
                    var repHeight = window.innerHeight / this.tileSide * this.height;
                    var repWidth = window.innerWidth / this.tileSide * this.width;
                    console.log(repHeight, repWidth);
                }
                */
        let indexTilesImg = 0;
        for (let j = 0; j < width; j++) {
            for (let i = 0; i < height; i++) {
                let posX = mapX + tileSide * i;
                let posY = mapY + tileSide * j;
...
var rectDivCirctuit = eDivCircuit.getBoundingClientRect();
var imgMap = [
...
var test = new tileMap(200, 200, 10, 10, 50, imgMap, true);
test.draw(eDivCircuit);

/*
Si besoin on peut scale en fonction de la taille de l ecran
           let repHeight = window.innerHeight / this.tileSide * this.height;
           let repWidth = window.innerWidth / this.tileSide * this.width;
           eImgTile.style.scale = "2";*/